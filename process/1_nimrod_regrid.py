...
def plot_data(rain_data, lat, lon, time, title, save):
    ...
    if save:
        ...
        plot_filename = '/home/daniele/Documenti/PhD_Cologne/Case_Studies/Germany_Flood_2021/rain_products/nimrod/images/rain_rate_'+date_string+'_'+title+'.png'
...
radar_folder = '/home/daniele/Documenti/PhD_Cologne/Case_Studies/Germany_Flood_2021/rain_products/nimrod/nc_files'  # Folder containing radar files
output_folder = '/home/daniele/Documenti/PhD_Cologne/Case_Studies/Germany_Flood_2021/rain_products/nimrod/nc_files_RegridToMSG/'

# Read MSG lat/lon data
msg_lat, msg_lon = read_msg_lat_lon(msg_file)
msg_lon_grid, msg_lat_grid = np.flip(msg_lon[0,:,:]), np.flip(msg_lat[0,:,:]) #np.meshgrid(msg_lon, msg_lat)
...
time, radar_lat, radar_lon, rain_rate = read_radar_data_with_lat_lon(radar_folder+'/nimrod_rain_data_eu_20210714_1200.nc')
...
for radar_file in radar_files:
    #radar_file = 'nimrod_rain_data_eu_20210714_1200.nc'
    ...
    if save:
        ...
        with Dataset(save_filename, 'w', format='NETCDF4') as nc:
            ...
            latitudes_nc = nc.createVariable('latitude', 'f4', ('x', 'y',))
            longitudes_nc = nc.createVariable('longitude', 'f4', ('x', 'y',))
            rain_rate_nc = nc.createVariable('rain_rate', 'f4', ('x', 'y',), fill_value=np.nan)
            ...
            #time = pd.Timestamp(str(time))
            # Convert to string in the format "yyyymmddhhmm"
